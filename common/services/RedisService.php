...
//                    $redis->auth($this->config['password']);
     ...
     * @param $newBetIds
     */
    public function removeNewBets($newBets)
    {
        $redis = $this->getRedis();
...
//        }
	 ...
	 * @return bool|int
	 */
	public function setDoSpiderAction($data = [])
	{
		if (empty($data)) {
			return false;
		}
		...
		$redis = $this->getRedis();
		return $redis->SET(RedisKeys::KEY_DOSPIDER_ACTION, json_encode($data));
	}
	 ...
	 * @return string
	 */
	public function getDoSpiderAction()
	{
		$redis = $this->getRedis();
		$data = $redis->get(RedisKeys::KEY_DOSPIDER_ACTION);
		return $data ? json_decode($data, true) : [];
	}
	 ...
	 * @return string
	 */
	public function removeDoSpiderAction()
	{
		$redis = $this->getRedis();
		return $redis->del(RedisKeys::KEY_DOSPIDER_ACTION);
	}
    /**
     * 红包竞猜次数并且设置过期时间
     * @return string
    ...
    public function incrPacketNumAction($key)
    {
        ...
        $key="packet_".$key;
        $num=$redis->incr($key);
        $time=strtotime(date("Y-m-d",strtotime("+1 days")))-1-time();//过期时间为第二天的0点
